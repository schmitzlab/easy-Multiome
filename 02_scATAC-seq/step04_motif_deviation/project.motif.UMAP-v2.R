...
#*************History*************
#2024/03/30 v2 updated to no upper limit for motif number. 120 motifs each plot.
...
clusterMotifs <- function(x, prefix="all"){
    ...
    write.table(cor.mat, file=paste0(prefix,".imputed.motif.correlations.txt"), quote=F, row.names=T, col.names=T, sep="\t")
...
#a$LouvainClusters <- a$leiden_refined
rownames(a) <- a$cellID
...
gids <- rownames(motif.sub.scores)

# plot
# plot multiple PNGs if length(gids) > 200
    if(num_motifs < 120){
    # setup plot size
#    nrows <- ceiling(num_motifs/6)
#    totals <- nrows*6
#    ratio <- nrows/6
    ...
    tnums <- nums*cols
    png(paste0(prefix,".motif.projections.imputed.UMAP.png"), width=(2*cols), height=(2*nums), units="in", res=500, type="cairo")
    layout(matrix(c(1:tnums), nrow=nums, ncol=5, byrow=T))
    par(mar=c(1,1,0.25,0.25))
    for(i in 1:num_motifs){
    cols <- colorRampPalette(c("dodgerblue4", "deepskyblue","grey90", "goldenrod2", "firebrick3"), bias=1)(100)
   ...
   dev.off()
    ...
    # split into groups
        gids.split <- ceiling(seq(1:num_motifs)/120)
        # iterate
        for(j in unique(gids.split)){
        
            # setup plot size
            gids2 <- gids[gids.split==j]
            #nrows <- ceiling(length(gids2)/5)
            #totals <- nrows*5
		    message("plotting motif scores on umap embeddings ...")
			
            cols <- 5
            nums <- ceiling(length(gids2)/5)
            tnums <- nums*cols
	    ratio <- tnums/5
#			png(file=paste0(outname,".",j,".png"), width=12, height=ratio*12, units="in", res=500, type="cairo")
            png(paste0(prefix,".",j,".motif.projections.imputed.UMAP.png"), width=(2*cols), height=(2*ratio), units="in", res=500, type="cairo")
            layout(matrix(c(1:tnums), nrow=nums, ncol=5, byrow=T))
            par(mar=c(1,1,0.25,0.25))
           for(i in 1:length(gids2)){
		      gene.index <- which(rownames(motif.sub.scores) == gids2[i])
			  #acv <- sub.scores[gene.index,]
              cols <- colorRampPalette(c("dodgerblue4", "deepskyblue","grey90", "goldenrod2", "firebrick3"), bias=1)(100)
              #cols <- colorRampPalette(viridis(100), bias=0.5)(100)
              #cols <- colorRampPalette(c("grey80","grey76","grey72",brewer.pal(9, "RdPu")[3:9]), bias=0.75)(100)
              scores <- as.numeric(motif.sub.scores[gene.index,])
              quants <- quantile(scores, c(0.02, 0.98))
              scores[scores > quants[2]] <- quants[2]
              scores[scores < quants[1]] <- quants[1]
              min.q <- quants[1] - abs(quants[1]*0.1)
              ...
              col.vec <- cols[cut(scores, breaks=seq(from=min.q, to=max.q, length.out=101))]
              plot(a$umap1, a$umap2, pch=16, col=col.vec, cex=0.3, 
                 xlab="", ylab="", xaxt="n", yaxt="n", main=rownames(motif.sub.scores)[gene.index],
                 bty="none")
             }
         dev.off()
   }
