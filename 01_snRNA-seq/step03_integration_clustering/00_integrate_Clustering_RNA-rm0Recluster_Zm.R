...
args <- commandArgs(T)
if(length(args) != 5){stop("Rscript snRNA-seq_clustering_xz.R <meta_path> <rds_path> <tissue> <prefix>")}

meta_path <- as.character(args[1])
rds_path <- as.character(args[2])
tissue <- as.character(args[3])
clean_meta <- as.character(args[4])
...
meta.c <- read.table(clean_meta)
meta.c <- meta.c[meta.c$celltype != "RNA_0",]
clean_cell <- meta.c$cellID

#get obj data
objs <- list.files(path=rds_path, pattern = "*.raw.seurat.rds", full.names=T)
objs <- objs[grepl(tissue, objs)]

#get meta data
metas <- list.files(path=meta_path, pattern = "*.updated_metadata_v2.txt", full.names=T)
...
objs <- lapply(objs, function(x){
  ...
  meta <- meta[rownames(meta) %in% clean_cell,]
  #update counts and meta
  counts <- GetAssayData(obj, slot="counts", assay="RNA")
  counts <- counts[!grepl("^Hetgly",rownames(counts)),] #filter nematode gene
  overlap <- intersect(rownames(meta), colnames(counts))
  counts <- counts[,overlap]
  meta <- meta[overlap,]
  # create new Seurat object
  obj.n <- CreateSeuratObject(counts=counts, project = name, meta.data=meta)
  return(obj.n)
})

saveRDS(objs, file=paste0(tissue,".seuratObjs.list.RDS"))

#objs <- readRDS("Gm_Infected_root.seuratObjs.list.RDS")
# normalize and filter doublets.
objs <- lapply(objs, function(x){
  obj <- x %>% 
    SCTransform(vars.to.regress = c('nUMI','pOrg')) %>% 
...
pdf(file=paste0(tissue,"_QC_violin_Rmdoublet.pdf"), height = 3, width = 3*number)
...
obj.m <- obj.m %>%
  RunUMAP(assay = "SCT",reduction = "harmony", dims = 1:25) %>%
  FindNeighbors(assay = "SCT",reduction = "harmony", dims = 1:25) %>%
  FindClusters(resolution = 0.5)
obj.m$tech <- "rna"
Idents(obj.m) <- obj.m$seurat_clusters
obj.m$celltype <- paste0("RNA_",Idents(obj.m))
saveRDS(obj.m, file=paste0(tissue,".MergedSeuratObjs.cluster.RDS"))
...
write.table(meta, file=paste0(prefix,".rna.Singlet.metadata.txt"), sep = "\t", quote=F)
...
pdf(file=paste0(prefix, '_Clustering_QC.pdf'), width = 12, height = 12)
...
p3 <- ggplot(meta, aes(x = seurat_clusters, y = pOrg, fill = library)) + 
  ...
  theme(legend.position='top')
#p4 <- ggplot(meta, aes(x = seurat_clusters, y = pNmt, fill = library)) + 
#  geom_violin(position = position_dodge(0.7), trim = FALSE) + 
#  geom_boxplot(position = position_dodge(0.7), color = "white", 
#               width = 0.05, show.legend = FALSE) + theme_classic()+xlab("") + ylab("Proportion of nematode") +
#  theme(legend.position='top')
