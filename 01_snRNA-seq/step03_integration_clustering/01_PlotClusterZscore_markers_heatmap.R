...
# a <- read.table("/scratch/xz24199/02Will82/scrna-seq/step03_ziliang/Gm_atlas_Root.rna.Singlet.metadata.txt")
# markers <- read.table("/scratch/xz24199/02Will82/scatac-seq/markers/At_atlas_marker_to_Gm_ALL.txt", header=T)
# b <- readRDS("/scratch/xz24199/02Will82/scrna-seq/step03_ziliang/Gm_atlas_Root.rna.sparse.rds")
# prefix <- "test_root"
# cluster_id <- "celltype"
...
# plot
write.table(fc.m, file=paste0(prefix,"_Log2FC_marker_genes.heatmap.txt"), quote=F, sep="\t")
pdf(paste0(prefix,"_Log2FC_marker_genes.heatmap.pdf"), width=8, height=20)
pheatmap(fc.m, col=colorRampPalette(rev(brewer.pal(9, "RdBu")))(100),
         cluster_col=F, cluster_row=F)
dev.off()

write.table(zscore.m, file=paste0(prefix,"_Zscore_marker_genes.heatmap.txt"), quote=F, sep="\t")
pdf(paste0(prefix,"_Zscore_marker_genes.heatmap.pdf"), width=8, height=20)
