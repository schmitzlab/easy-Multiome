...
#**********History********
# 2023/09/28 v2 output multiple plot if more than 120 genes.
# 2023/10/12 v3 add column name parameter wich contain the cluster id.
...
plot.act.scores    <- function(df, 
	...
	colvec[is.na(colvec) & acv == 0] <- cols[1]
		...
		# plot
            plot(df2$umap1, df2$umap2, col=colvec,
                 main=paste(info$name[i],info$type[i],info$geneID[i],sep="-"),cex.main=0.6,
                 xlab="", ylab="", bty="n",
                 xaxt="n", yaxt="n", pch=16, cex=0.5)
    }
   ...
   # split into groups
        gids.split <- ceiling(seq(1:length(gids))/120)
        ...
        for(j in unique(gids.split)){
            ...
            for (i in 1:length(gids2)){
                ...
                # copy meta data
                gene.index <- which(rownames(sub.scores) == gids2[i])
                info2 <- info[gids2,]
                acv <- sub.scores[gene.index,]
                ...
                acv[acv > upper.lim] <- upper.lim
                if(!is.null(marker.dist)){
                    message(" - # cells = ", length(acv), "| min: ", marker.dist[[gids2[i]]][1], " | max: ",marker.dist[[gids2[i]]][2])
                    colvec <- cols[cut(acv, breaks=seq(from=marker.dist[[gids2[i]]][1], to=marker.dist[[gids2[i]]][2], length.out=101))]
                }else{
                    min.acv <- min(acv) - (1e-6*min(acv))
                    max.acv <- max(acv) + (1e-6*max(acv))
                    message(" - # cells = ", length(acv), "| min: ", min.acv, " | max: ",max.acv)
                ...
                # plot
                plot(df2$umap1, df2$umap2, col=colvec,
                     main=paste(info2$name[i],info2$type[i],info2$geneID[i],sep="-"),cex.main=0.6,
                     xlab="", ylab="", bty="n",
                     xaxt="n", yaxt="n", pch=16, cex=0.5)
