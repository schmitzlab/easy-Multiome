...
plotDist   <- function(x, main=""){
    ...
    df.n <- df.n[3000:15000,]
    df.n <- subset(df.n, df.n$log10depth >= 2 & df.n$log10depth <= 4)
...
pdf(paste(name,".QC_FIGURES.pdf",sep = ""), width=12, height=4)
...
#    meta.v2$prop <- meta.v2$tss/meta.v2$unique
    meta.v1$pOrg_z <- as.numeric(scale(meta.v1$pOrg))
    meta.v1 <- meta.v1[order(meta.v1$pOrg_z, decreasing=T),]
    below <- subset(meta.v1, meta.v1$pOrg_z > 2)
    if(!is.null(below)){
	    thresh <- min(0.2, min(below$pOrg))
    }else{
	    thresh <- 0.2
    }
    meta.v1 <- meta.v1[order(meta.v1$unique, decreasing=T),]
    meta.v2 <- subset(meta.v1, meta.v1$pOrg <= thresh)
#    threshs <- c(threshs, thresh)
    den <- kde2d(log10(meta.v1$unique), meta.v1$pOrg, 
                 n=300, h=c(0.2, 0.05),
                 lims=c(c(2, 6.5), c(0,1)))
    image(den, useRaster=T, col=c("white", rev(magma(100))),
          xlab="Number of UMI (log10)", ylab="Fraction reads in organelle",
          main=name)
    grid(lty=1, lwd=0.5, col="grey90")
    abline(h=thresh, col="red", lty=2, lwd=1)
...
message(" - number of cells after pOrg filter v2 = ",nrow(meta.v2))
write.table(meta.v2, file=paste(name,".updated_metadata_v2.txt", sep = ""), sep="\t", quote=F, row.names=T, col.names=T)

###### plot spliced rate and filter cells ###########
#    meta.v2$prop <- meta.v2$tss/meta.v2$unique
    meta.v2$spliced_rate_z <- as.numeric(scale(meta.v2$spliced_rate))
    meta.v2 <- meta.v2[order(meta.v2$spliced_rate_z, decreasing=T),]
    below <- subset(meta.v2, meta.v2$spliced_rate_z > 2)
    if(!is.null(below)){
	    thresh <- min(0.8, min(below$spliced_rate))
    }else{
	    thresh <- 0.8
    }
    meta.v2 <- meta.v2[order(meta.v2$unique, decreasing=T),]
    meta.v3 <- subset(meta.v2, meta.v2$spliced_rate <= thresh)
#    threshs <- c(threshs, thresh)
    den <- kde2d(log10(meta.v2$unique), meta.v2$spliced_rate, 
                 n=300, h=c(0.2, 0.05),
                 lims=c(c(2, 6.5), c(0,1)))
    image(den, useRaster=T, col=c("white", rev(magma(100))),
          xlab="Number of gene (log10)", ylab="Fraction spliced reads",
          main=name)
    grid(lty=1, lwd=0.5, col="grey90")
    abline(h=thresh, col="red", lty=2, lwd=1)
    legend("topright", legend=paste("# cells = ", nrow(meta.v3), sep=""), fill=NA, col=NA, border=NA)
...
message(" - number of cells after spliced_rate filter v3 = ",nrow(meta.v3))
write.table(meta.v3, file=paste(name,".updated_metadata_v3.txt", sep = ""), sep="\t", quote=F, row.names=T, col.names=T)


##### filter the clean cell with call column ###########
#meta.v5 <- meta.v4[meta.v4$call == 1,]
#meta.v4 <- subset(meta.v3, call == 1)
#message(" - number of cells after filter cleanCell v3 = ",nrow(meta.v3))
